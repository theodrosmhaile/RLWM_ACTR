...
### meta learning group
sdat.meta <-  read.csv('meta_learning_group.csv', header = T)

#### Combine the two groups
subjects #%>% 
  cbind(subjects %>% c(), sdat.meta[,1] %>% c() )

rbind(sdat.mod, sdat.meta)
...
```{r models + subjects }
mod.sub.learn <- moded.cols=c('#f46d43','#4d9221', '#d73027', '#2166ac', '#4d9221') 

mods.subs.learn <- melted.p.behav.model %>% 
   filter(iteration != 13) %>% 
