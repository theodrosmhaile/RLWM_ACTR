...
#Import data
wmodata     <- read.csv('./processed/wmo_subjects_across_studies_learn_031820.csv') 
wmotestdata <- read.csv('./processed/wmo_subjects_across_studies_test_031820.csv') 
...
Subjects <- as.matrix(read.csv('wmo_subjects_across_studies_031820.csv',header = FALSE))
```
...
Test.Data  %>% gather(key='study', value = 'acc', -iteration) %>% view()
...
for (s in Subjects) {
  OneSub <- wmotestdata$subject==s  
  ns3.test.mean[i] <- wmotestdata[!is.na(match(wmotestdata[,"stim"],block.3)) & OneSub,"acc"]
  ns6.test.mean[i] <- mean(wmotestdata[!is.na(match(wmotestdata[,"stim"],block.6)) & OneSub,"acc"])

  i = i + 1
}

TestData = cbind('n3.test'=unlist(ns3.test.mean) %>% as.numeric(),
                      'n6.test'=unlist(ns6.test.mean) %>% as.numeric())



#plot
TestData %>% gather(key='set_size', value = 'Accuracy') %>% 
  ggplot( aes(set_size,Accuracy, color=set_size)) + 
  geom_bar(s) + 
  xlab("Set size") + 
  scale_colour_brewer( palette = "Set1") +
  #ylim(c(0.2,1))+
  theme_minimal(base_size = 20)
tmp = c('#ca0020','#0571b0','#92c5de')
bar.names=c("set 3", "set 6")

TestData %>% melt() %>% 
  summarySE(measurevar = 'value', groupvars = c('Var2')) %>% 
  ggplot(aes(Var2, value, fill=Var2))+
 ...
 scale_fill_manual(values = tmp, guide='none')+
  ylab('Accuracy')+
  xlab('condition')+
  scale_x_discrete(labels=bar.names )+
...
#----- Save learn-test data for all subjects for model testing
  write.csv(rbind(mean.acc.3, mean.acc.6, ns3.test.mean, ns6.test.mean), 'all_subject_n83_learn_test_data.csv', row.names = FALSE, col.names = FALSE)  
