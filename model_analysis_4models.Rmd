...
simsRL_LTMorig <- fromJSON('RLLTMorig_sim_data050520.JSON')
...
simsRL_LTMorig.s3s6test.temp <-
  ...
  cbind(., simsRL_LTMorig$data$set6_test) %>% 
   rep(., 12) %>%
   as.matrix() %>%
   reshape(., row(simsRL_LTMorig$data)*12, 2) %>%
   reshape(., row(simsRL_LTMorig$data)*2, 12) 
...
simsRL_LTMpipe <- fromJSON('RLLTM2_pipe_sim_data050420.JSON')
...
simsRL_LTMstr <- fromJSON('RLLTM1_strategy_sim_data050420.JSON')
...
simsRL <- fromJSON('RL_sim_data_3lvsparams_050420.JSON')
...
simsRL.set6learn <- simsRL$data$set6_learn %>% 
 ...
 RL.sim.dat <- cbind(simsRL.set3learn, 
                     ...
                     simsRL.s3s6test.temp[9:18, ] )

#--------- Longterm Memory/WM/Declarative model 
 
simsLTM    <- fromJSON('LTM_sim_data_3lvparams_050420.JSON') 
...
for(s in c(1:nrow(sdat.mod))) { # for each subject
 ...
 mseRL_LTMorig.temp <- rbind(mseRL_LTMorig.temp, 
                             apply(RL_LTMorig.sim.dat, 1, function(x,y) MSE(x, sdat.mod[s, ])))
 # model 3.1
 mseRL_LTMpipe.temp <- rbind(mseRL_LTMpipe.temp, 
                             apply(RL_LTMpipe.sim.dat, 1, function(x,y) MSE(x, sdat.mod[s, ])))
 # model 3.2
mseRL_LTMstr.temp  <- rbind(mseRL_LTMstr.temp, 
                            apply(RL_LTMstr.sim.dat, 1, function(x,y) MSE(x, sdat.mod[s, ])))
