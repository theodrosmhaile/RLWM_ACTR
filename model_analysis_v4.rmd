...
consist.check <- rbind(
  ...
  RL_LTMpipe.bic %>% t() %>% data.frame(mod='Meta_RL'),
...
BIC.diff <-  
   apply(sorted.bic.dat %>%  data.frame(ind=c(1:15775)) %>% filter(ind<3),2,diff) %>% 
...
mod.pref %>% 
  ggplot(aes(y=mod.type, x=model, fill=prop2))+
  geom_tile()  +
    geom_text(aes(label= paste0(round(prop2* 100 ,2),'%'), 
                                size=.5),
                  show.legend = F) +
