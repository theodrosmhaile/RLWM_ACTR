...
simsRL_LTMpipe <- fromJSON('./simulated_data/pipe_model/pipe_sim_data_12292020_corrected.JSON')
...
simsRL_LTMstr <- fromJSON('./simulated_data/strategy_model/strategy_sim_data_12262020_corrected.JSON')
...
simsRL <- fromJSON('./simulated_data/RL_model/RL_simulated_data_12292020_corrected.JSON')
...
simsLTM    <- fromJSON('./simulated_data/LTM_model/LTM_sim_data_122020_corrected.JSON')
...
```{r models only, fig.width=12, fig.height=12,fig.cap = "Figure 4."}
...
if(0){
 ...
 #stats
  all.models %>% 
    separate("condition", into = c( 'setSize','iteration'), convert = T) %>%   
    filter(iteration==12) %>% 
  summarySE(measurevar = 'accuracy', groupvars = c('model','setSize')) %>% kable()
...
```{r subjects only, fig.width=12, fig.height=12, fig.cap="Figure 5." }
 melted.p.behav.model %>% 
   filter(iteration != 13) %>% 
...
subjects.only.plt <- melted.p.behav.model %>% 
   filter(iteration != 13) %>% 
...
```{r Learning and test combined plot, fig.width=12, fig.height=12,fig.cap="figure 6"}
...
```{r Learning rate 1,fig.width=12, fig.height=12, fig.cap="Figure 7."}
...
```{r alternative learning rate,fig.width=12, fig.height=12, fig.cap='Figure 8.'}
...
```{r Analysis of separation between the curves, fig.cap="Figure 9."}
...
```{r Change from learning to test, fig.cap="Figure 10."}
...
```{r summarize extracted parameters, fig.cap="Figure 11."}
...
```{r effects of individual parameters on outcomes,fig.width=12,fig.height=4, fig.cap="Figure 12"}
