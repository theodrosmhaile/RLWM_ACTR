---
title: "RLWM ACT-R RMSE model fitting and outcome analysis"
author: "Theodros H."
date: "07/2022"
output:
  html_document:
...
#fromJSON('./simulated_data/pipe_model/pipe_sim_data_12292020_corrected.JSON')
  #fromJSON('./simulated_data/pipe_model/sim_data_072020/pipe_all_data_5params_071320.JSON')
...
simsRL_LTMpipe.s3s6test.temp <-
 ...
 RL_LTMpipe.sim.dat <- cbind(simsRL_LTMpipe.set3learn, 
                         ...
                         simsRL_LTMpipe$data[,c('bll','alpha','egs','imag','ans','strtg', 'strtg3','strtg6')]) %>% 
   data.table()

 
 #--------- Integrated model assigned strategy
simsRL_LTMstr <- fromJSON('./simulated_data/strategy_model/STR_sim_data_032021.JSON')
#fromJSON('./simulated_data/strategy_model/strategy_sim_data_12262020_corrected.JSON')
  #fromJSON('./simulated_data/strategy_model/sim_data_072020/strategy_all_data_5params_071320.JSON')
...
#fromJSON('./simulated_data/RL_model/RL_simulated_data_12292020_corrected.JSON')
...
#fromJSON('./simulated_data/LTM_model/LTM_sim_data_122020_corrected.JSON')
  #fromJSON('./simulated_data/LTM_model/sim_data_072020/LTM_all_data_5params_071320.JSON')
...
## LTM subjects
  ...
  RMSE.LTM <- tibble(subjects =subjects[participants.fit==2], 
                     RMSE = NA)
  LTM_sub_id = c(1:83)[participants.fit==2]
  
  for (i in 1:length(LTM_sub_id)) {
    
    RMSE.LTM$RMSE[i] = mseLTM.temp[i, ind.temp.LTM[participants.fit==2][i]]
  }
## RL subjects
  RMSE.RL <- tibble(subjects =subjects[participants.fit==1], 
                     RMSE = NA)
  RL_sub_id = c(1:83)[participants.fit==1]
  
  for (i in 1:length(RL_sub_id)) {
    
    RMSE.RL$RMSE[i] = mseRL.temp[i, ind.temp.RL[participants.fit==1][i]]
  }

## RL_LTM pipe subjects
  RMSE.RL_LTMpipe <- tibble(subjects =subjects[participants.fit==3], 
                     RMSE = NA)
  pipe_sub_id = c(1:83)[participants.fit==3]
  
  for (i in 1:length(pipe_sub_id)) {
    
    RMSE.RL_LTMpipe$RMSE[i] = mseRL_LTMpipe.temp[i, ind.temp.RL_LTMpipe[participants.fit==3][i]]
  }
  
  ## RL_LTM STR subjects
  RMSE.RL_LTMstr <- tibble(subjects =subjects[participants.fit==4], 
                     RMSE = NA)
  str_sub_id = c(1:83)[participants.fit==4]
  
  for (i in 1:length(str_sub_id)) {
    
    RMSE.RL_LTMstr$RMSE[i] = mseRL_LTMstr.temp[i, ind.temp.RL_LTMstr[participants.fit==4][i]]
  }
  
  all_best_fit_RMSE <-  rbind(RMSE.RL, RMSE.LTM, RMSE.RL_LTMpipe, RMSE.RL_LTMstr)
  
  write_csv(all_best_fit_RMSE, file = 'bestFit_model_RMSE.csv')
...
pipe.p.model <- data.frame( 'subjects' = subjects[participants.fit==3],
                            'model' = rep('Meta-RL', sum(participants.fit==3)),
...
```
 ...
 Of the four models compared, the LTM model fit the most number of participants (`r sum(participants.fit==2)`) followed by the biased version of the combined RL-LTM model (`r sum(participants.fit==4)`) and the meta-RL combined model in third place (`r sum(participants.fit==3)`). The RL only model had only `r sum(participants.fit==1)` participant(s) that fit it best (figure 1). This is a slight departure from out expectation that the combined RL-LTM models would fit the majority of participants. As observed, this suggests that most learners simply commit to memory the stimulus response associations. 
...
consist.check <- rbind(
  ...
  RL_LTMpipe.bic %>% t() %>% data.frame(mod='Meta.RL'),
  RL_LTMstr.bic %>% t() %>% data.frame(mod='Biased')
)

ordered.dat=apply(consist.check[,1:n_subs], 2, order)
sorted.bic.dat <- apply(consist.check[,1:n_subs], 2, sort)
mod.list <- (rep(consist.check$mod,n_subs))
comp.temp <- mod.list %>% as.matrix() %>%  matlab::reshape(.,length(consist.check$mod),n_subs)

comp <- comp.temp[ordered.dat[,c(1:n_subs)]] %>% 
  as.matrix() %>% matlab::reshape(.,length(consist.check$mod),n_subs)
...
geom_point() +
  ylab("mean BIC") +
  xlab("ranked index of consecutive model fits")+
...
BIC.diff %>% 
  filter(ind2<11) %>% 
   group_by(ind2) %>% 
  dplyr::summarise('mean.diff'=mean(value),'sem'=sd(value)/sqrt(n_subs)) %>% 
...
BIC.diff %>% 
 ...
 # filter(model=='LTM') %>% 
  ...
  dplyr::summarise(mean=mean(BIC.diff),SD=sd(BIC.diff), 
                   'sem'=sd(BIC.diff)/sqrt(length(BIC.diff)),
                   'N' = length(BIC.diff)
                   ) 
...
mod.pref.temp <- 
  comp %>% 
    data.frame(ind=c(1:15775)) %>% 
  filter(ind<11) %>%
  pivot_longer(cols = c(-ind), names_to ='subjects', values_to = 'fit.models' ) %>% 
  mutate(best.model = rep(comp[1,],10)
         ) %>% 
    group_by (best.model)  %>% 
    dplyr::count(fit.models) %>% 
  
    group_by(best.model, .drop = F) 

mod.pref <- 
  mod.pref.temp %>% 
    summarise(totals=sum(n)) %>% 
  inner_join( mod.pref.temp, by = 'best.model') %>% 
    group_by(best.model, fit.models) %>% 
    summarise(prop= n/totals) %>% 
   ungroup() %>% 
   add_row(best.model=c('Biased','Meta.RL','RL','RL' ), 
           fit.models=c('RL','RL', 'Biased','Meta.RL'), 
           prop=c(0,0,0,0)
           )
 ...
 #                   n=sum(!is.na(value)),
 #                   sem=sd(value,na.rm = T)/sqrt(sum(!is.na(value))),
 #                   prop=mean(value, na.rm = T)/10) %>% 
 #  mutate(prop2=case_when(is.nan(prop)==T ~ 0,
 #                         is.nan(prop)==F ~ prop))
 #  

mod.pref %>% 
  ggplot(aes(y=fit.models, x=best.model, fill=prop))+
  geom_tile()  +
  ...
  ylab('Available models') +
 labs(fill='Percentage') +
 guides(fill = guide_colorbar(title.position = "top", barwidth = 18)) +
  scale_x_discrete(labels=c('Biased', 'LTM', 'Meta-RL', 'RL'))+
  scale_y_discrete(labels=c('Biased', 'LTM', 'Meta-RL', 'RL'))+
  theme_pubclean(base_size = 28)
# 
# mod.pref %>% mutate(pref=(model==mod.type)) %>% filter(pref)
# ##Chantel's request 
#    BIC.diff %>% 
#   filter(ind2<4) %>% 
#    reshape2::dcast(variable ~ ind2, value.var = 'value') %>% 
...
all.models.long.plt %>% 
  filter(iteration >=12) %>% 
  summarySE(measurevar = 'accuracy', groupvars = c('model','setSize','iteration', 'cond.model')) %>% filter(iteration==12) %>% 
  ggplot(aes(as.factor(iteration),accuracy, group=cond.model,color=cond.model)) +
  geom_point(size=1.5) +
...
```{r models + subjects }
 moded.cols=c('#f46d43','#4d9221', '#d73027', '#2166ac', '#4d9221') 
...
mods.subs.learn <- 
  ...
  facet_wrap(vars(model), ncol = 1, strip.position =  'left') +
  scale_color_brewer(palette = "Paired") +
  #scale_color_manual(values = moded.cols)+
  xlab('stimulus iteration') +
 theme_pubclean(base_size = 20)  +
  theme(legend.position = 'none')
```  
...
```{r Learning and test combined plot}
...
#   facet_wrap(vars(setSize))
  ...
  select(setSize, accuracy, iteration) %>% 
    mutate(iter=as.factor(iteration)) %>% 
    lm(accuracy ~ setSize * iteration, data=.) %>% 
  anova() %>%  
    broom::tidy() %>% kable(caption = "2 x 2 setzise by condition(learn vs test) ANOVA table for behavioral data")
  
  
  ## diffrence between learning and test for behav data by model group
  
  melted.p.behav.model %>% 
  filter(type=='behav', iteration>=12) %>% 
    lm(accuracy ~ model* iteration, data = .) %>% 
    anova()
    
  
  #compare learn and test differences by model type to show that all models capture the difference but only the Meta-RL model people show this difference
   melted.p.behav.model %>% 
  filter(type=='behav', iteration==13, model=='Meta-RL') %>% 
    wilcox.test(accuracy~setSize, data = ., exact=FALSE)
    
   melted.p.behav.model %>% 
  filter(type=='behav', iteration==13, model=='RL') %>% 
    wilcox.test(accuracy~setSize, data = ., exact=FALSE)
  
   melted.p.behav.model %>% 
  filter(type=='behav', iteration==13, model=='LTM') %>% 
    wilcox.test(accuracy~setSize, data = ., exact=FALSE)
   
    melted.p.behav.model %>% 
  filter(type=='behav', iteration==13, model=='Biased') %>% 
    wilcox.test(accuracy~setSize, data = ., exact=FALSE)
  ...
  select(setSize, accuracy, iteration) %>% 
    lm(accuracy ~ setSize * iteration, data=.) %>% 
  anova() %>%  
    broom::tidy() %>% kable(caption = "2 x 2 setzise by condition(learn vs test) ANOVA table for model data")
#   
#   
# melted.p.behav.model %>% 
#   filter(type=='behav', iteration==13) %>% 
#  # select(accuracy,model) %$% 
# pairwise.t.test(accuracy ~ model, p.adjust.method = 'bonferroni', exact=FALSE) %>% 
#   broom::tidy() %>% 
#     kable(caption = 'pairwise post-hoc tests for behav data')   
  
```

<!-- ```{r plot models + subjects fig.width=6, fig.height=12, fig.cap="Figure 8."} -->
<!-- ggarrange(mods.subs.learn, learn2test.plt) -->
<!-- ``` -->
...
melted.p.behav.model %>% 
  ...
  select(setSize, accuracy, iteration) %>%
  mutate(iter=as.factor(iteration)) %>% 
    lm(accuracy ~ setSize * iter, data=.) %>% 
  anova() %>%  
    broom::tidy() %>% kable(caption = "2 x 2 setzise by iteration ANOVA table for behavioral data")

melted.p.behav.model %>% 
  filter(type=='behav', iteration==12) %>% 
  select(setSize, accuracy, iteration) %>%
  mutate(iter=as.factor(iteration)) %>% 
  t.test(accuracy ~setSize, data=.) %>% 
    broom::tidy() %>% kable(caption = "wilcoxon test for set-zise difference in accuracy behavioral data")
...
```{r learning rate,fig.width=12, fig.height=12, fig.cap='Figure 11.'}
...
fit.lr <-  melted.p.behav.model %>% 
    ...
    do( broom::tidy(lm(accuracy~iteration, data=.))[2] %>% 
          as_data_frame()) %>% 
   cbind('estimate.type'= rep(c('y-int','slope'), n_subs*2*2)) 
 ...
 fit.lr.plt <- 
  ...
  geom_point(size=2)+
   ...
   ylab("slope") +
    xlab('set-size')+
   theme(legend.position = 'none')
  # ggtitle("learning rate: slope fit to first 6 presentations")
 ...
 fit.lr %>%
   ...
   kable(caption = 'mean and median of slope for behavioral data by set-size')
...
#    kable(caption = 't-test: learning rate differences between the 2 set-sizes')
#     
# fit.lr %>% 
#    ungroup() %>% 
#    dplyr::filter(type=='behav', estimate.type=='slope') %>% 
#    select(setSize, estimate, model) %>%
#    lm(estimate ~ setSize*model, data = .) %>% 
#   anova()
#    kable(caption = 't-test: learning rate differences among the model groups')
#     
# 
#    
#    fit.lr %>% 
#    filter(estimate.type=='slope', type=='behav') %>% 
#    group_by(setSize,type, model) %>%
#    dplyr::summarize('mean'= mean(estimate),
#                     'se' = sd(estimate, na.rm = T) / sqrt(sum(!is.na(estimate)))) %>% 
...
#    
  ...
  #     #geom_density(alpha=.4)
  #     lm(estimate ~ type*model*setSize,data=.) %>% 
  #     anova()
  # 
  #     
  #    
...
s3s6Diff %>% 
  group_by(subjects,model, type) %>% 
 summarize(diff.mean = mean(learnDiff)) %>% 
 # filter(model!='RL') %>% 
   lm(diff.mean ~ model*type, data=. ) %>%
  anova()
  broom::tidy() 
  ...
  group_by(subjects,model, type) %>% 
 summarize(diff.mean = mean(learnDiff)) %>% 
  filter(model=='Meta-RL', type=='behav') %$% 
    t.test(diff.mean, mu=0, exact= F)
    
  s3s6Diff %>% 
  group_by(subjects,model, type) %>% 
 summarize(diff.mean = mean(learnDiff)) %>% 
  filter(model=='Biased', type=='behav') %$% 
    t.test(diff.mean, mu=0)
 
  s3s6Diff %>% 
  group_by(subjects,model, type) %>% 
 summarize(diff.mean = mean(learnDiff)) %>% 
  filter(model=='LTM', type=='behav') %$% 
    t.test(diff.mean, mu=0)
 ...
 summarize(diff.mean = mean(learnDiff)) %>% 
  ...
  group_by(subjects,model, type) %>% 
 summarize(diff.mean = mean(learnDiff)) %>% 
  filter(model=='Meta-RL', type=='model') %$% 
    t.test(diff.mean, mu=0, exact= F)
  ...
  group_by(subjects,model, type) %>% 
 summarize(diff.mean = mean(learnDiff)) %>% 
  filter(model=='Biased', type=='model') %$% 
    t.test(diff.mean, mu=0)
  ...
  group_by(subjects,model, type) %>% 
 summarize(diff.mean = mean(learnDiff)) %>% 
  filter(model=='LTM', type=='model') %$% 
    t.test(diff.mean, mu=0)
  #diffs for RL
 #   s3s6Diff %>% 
 #  group_by(subjects,model, type) %>% 
 # summarize(diff.mean = mean(learnDiff)) %>% 
 #  filter(model=='RL', type=='model') %$% 
 #    t.test(diff.mean, mu=0)
...
s3s6Diff %>% 
 ...
 summarize(diff.mean = mean(learnDiff)) %>% 
  filter(model!='RL', type=='behav') %>% 
  select(diff.mean,model)%$%
    pairwise.wilcox.test(diff.mean,model,p.adjust.method = 'bonferroni', data=.) %>% 
      broom::tidy()
...
```{r Change from learning to test, fig.cap="Figure 13."}
   ...
   learnTestDiff <- 
     melted.p.behav.model %>% 
       filter( iteration>= 12) %>% 
        select(c('subjects','model', 'accuracy','iteration', 'setSize','type', cond.model)) %>% 
   pivot_wider(names_from = iteration, values_from = accuracy) %>% 
  mutate('test_learn' = `13`-`12`) 
  ...
  ggplot(aes(model, test_learn, color=cond.model, group=cond.model)) + 
      geom_point(position=position_dodge(width = 0.4),size=2) + 
      geom_errorbar(position=position_dodge(width = 0.4),aes(ymax=test_learn+se, ymin=test_learn-se), size=1,width=.75) +
      theme_pubclean(base_size = 24) +
    #  ggtitle("change from learning to test: test - learn")+
        scale_color_brewer(palette = "Paired") +
      theme(legend.position = 'none')+
      ylab("mean difference")
    #  ylim(c(-0.6,0))
  ...
  learnTestDiff %>% 
    filter(model!='RL') %>% 
    lm(test_learn ~ setSize * type * model, data=.) %>% 
    anova() %>% 
    broom::tidy() %>% 
  ...
  learnTestDiff %>% 
    filter(model=='LTM', type=='behav') %>%
    group_by(setSize) %>% 
    summarise(mean(test_learn))
```

```{r learing features panel plot, fig.cap="Figure 10", fig.height=12, fig.width=12}
ggarrange(s3s6Diff.plt,learn_test.plt , 
          labels = 'AUTO', 
...
param.spread <- 
  ...
  data.frame('scaled.vals' =p.behav.model %>% select('alpha','egs','bll','imag','ans','bias') %>% 
               scale() %>% 
               reshape2::melt()  %>%
               select(value) ) %>% 
...
```{r example plots of individuals}
rad.plt =  p.behav.model %>% 
  select(subjects, 'alpha','egs','bll','imag','ans','bias') %>% 
   pivot_longer(cols =-subjects, 
                names_to = 'parameters', 
                values_to = 'unscaled.vals' ) %>%
  inner_join( tibble(
   p.behav.model %>% 
      select('alpha','egs','bll','imag','ans','bias') %>% 
      scale() %>% as.data.frame(),
   p.behav.model %>% 
      select(subjects)) %>% 
      pivot_longer(cols = -subjects, names_to='parameters', values_to ='scaled' ),
    by = c('subjects', 'parameters'))


  
  rad.plt %>% 
    filter(subjects==29239 |subjects==6247 | subjects==6242  ) %>% 
    ggplot() +
    geom_hline(aes(yintercept = rep(c(1:.7)/10, 498/10)[1:18]  ), 
               color = "lightgrey"
                 ) +
    geom_col(
    aes(
...
melted.p.behav.model %>% 
  filter(type=='behav', model=='Biased', iteration==1, setSize=='s3') %>% 
  select(subjects, bias) %>% 
 mutate(prop=bias<.5) %>% 
  filter(prop) %>% 
  summarise(mean(bias), sem=sd(bias)/sqrt(length(bias)), n=(length(bias)))


#Meta-RL bias differences
melted.p.behav.model %>% 
  filter(type=='behav', model=='Meta-RL', iteration==1, setSize=='s3') %>% 
  select(subjects) %>% 
mutate('b3'=RL_LTMpipe.sim.dat[ind.temp.RL_LTMpipe[participants.fit==3],55]%>% reduce(rbind),
       'b6'=RL_LTMpipe.sim.dat[ind.temp.RL_LTMpipe[participants.fit==3],56]%>% reduce(rbind)) %>% 
  pivot_longer(cols= -subjects, names_to = 'set', values_to = 'param.vals') %>% 
  group_by(set) %>% 
  summarise(mean=mean(param.vals), sem=sd(param.vals)/sqrt(length(param.vals)))
...
# uclimb <- read_csv("RLWM_data/python_olcts_uclimb_summary_noEEG_092320.csv")
# uclimb.model.temp1 <- uclimb %>%  merge(p.behav.model, by = "subjects")
# uclimb.model.dat <-  cbind(uclimb.model.temp1, 'wmComposite'= uclimb.model.temp1[,c("RspanScore", "SspanAbsoluteScore","OspanAbsoluteScore") ] %>% rowMeans()) %>% as_tibble()
# 
...
# #7 PSS measures
# uclimb.model.dat %>%
#   dplyr::select('MLAT_Total', `Ravens Score`,"NDV", "wmComposite","RspanTotal", "PSS_ChooseA.ACC" , "PSS_AvoidB.ACC", "PSS_All.ACC" ) %>%
#   scale(center = F)  %>%
...
# 
# uclimb.python.dat %>%
#   dplyr::select( "PostTest_Total_ACC","python.learning.coeff","PT_Syntax_ACC" ,"PT_Semantics_ACC","RPS.Code") %>%
#   scale(center = F)  %>%
...
plot.indiv('Biased', 'Integrated Model - Biased', 4)

```

```{r indiv plots, metaRL, fig.height=15, fig.width=24 }
plot.indiv('Meta-RL', 'Integrated Model - Meta RL', 4)
```
