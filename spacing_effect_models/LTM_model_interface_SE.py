...
# ### 07/26/2023 current version includes a 28 minute break instead of the standard break
...
actr.load_act_r_model(os.path.join(curr_dir, "LTM_model_SE.lisp")) #integrated-model-pipe.lisp
...
def present_feedback():
    ...
    # present feedback
        ...
        if i == lastLearnTrial:
            #rint("BREAK HERE")
            actr.schedule_event_relative(1680, 'present_stim') #600
        else:
            actr.schedule_event_relative(1, 'present_stim')
...
#bll_param   = [0.3, 0.4, 0.5, 0.6, 0.7]   # decay rate of declarative memory,range around .5 actr rec val replaced by alpha
...
ans_param   = [0.5, 0.6, 0.7, 0.8, 0.9] #updated noise parameter to counteract the ceiling effect obtained using the Pavlik equation. 
...
#params = [se_param, alpha_param, egs_param, imag_param, ans_param]
...
# LTM model params
params = [se_param, imag_param, ans_param]
param_combs = list(itertools.product(*params))
...
def simulation(se, alpha, egs, imag, ans, nSims):
...
#                   save averaged resluts from simulations along with parameters
    ...
    sim_data.append([np.mean(temp3,0), np.mean(temp6,0), np.mean(np.mean(temp_test3,1)), np.mean(np.mean(temp_test6, 1)), se, alpha, egs, imag, ans ])
...
def execute_sim(n,fromI,toI, frac):
    ...
    sim.to_json('./model_sims/LTM_sim_data_' + 'frac_2_66_to_150.JSON', orient='table')

    sim = pd.DataFrame(sim_data, columns=['set3_learn','set6_learn', 'set3_test', 'set6_test','se', 'alpha', 'egs', 'imag', 'ans' ])
    sim_st = pd.DataFrame(sim_std, columns=['set3_learn','set6_learn', 'set3_test', 'set6_test'])
    sim_st.to_json('./model_sims/LTM_sim_std_date.JSON',orient='table')
    sim.to_json('./model_sims/LTM_sim_data_' + 'frac_' +np.str(frac) +'_'+ np.str(fromI) + '_to_' + np.str(toI) + '.JSON', orient='table')
