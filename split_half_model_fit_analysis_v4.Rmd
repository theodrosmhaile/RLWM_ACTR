...
LTM.BIC.S3 <-  fit.models.setsize(model = 'LTM', setsize = 3, 3)
LTM.BIC.S6 <-  fit.models.setsize(model = 'LTM', setsize = 6, 3)


##########---------RL-LTMstr FITS ----------------############

STR.BIC.S3 <-  fit.models.setsize(model = 'STR', setsize = 3, 6)
STR.BIC.S6 <-  fit.models.setsize(model = 'STR', setsize = 6, 6)


##########---------RL-LTMmeta FITS ----------------############
META.BIC.S3 <-  fit.models.setsize(model = 'META', setsize = 3, 5)
META.BIC.S6 <-  fit.models.setsize(model = 'META', setsize = 6, 5)
...
```{r comparison of the bias parameter}
bias_analysis <- 
best.fits.ss %>% 
  inner_join(best.fit.idx.ss,
             by=c('setsize', 'subjects')) %>% 
  filter(model=='STR') %>% 
  inner_join(STR.sim %>% 
               select(index, bias), 
             by = 'index') %>% 
  inner_join( best.fits.ss %>% 
                filter(model=='STR') %>% 
                pivot_wider(id_cols = c(subjects), names_from = setsize, values_from = model) %>%
                mutate(same = !is.na(N3==N6)) %>% 
                select(subjects, same), 
              by = 'subjects')


bias_analysis %>% 
  #filter(same==F) %>% 
  group_by(same, setsize ) %>% 
...
mean.data %>% 
  ...
  summarise(mean.accuracy = mean(acc.mean), 
            n=n(), 
            se = sd(acc.mean)/sqrt(n)
            ) %>% 
  ggplot(aes(x=stable, y=mean.accuracy,
