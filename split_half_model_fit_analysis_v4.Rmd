...
#### does accuracy matter interms of which model fit participants in half 2. 
acc.half.model %>% 
  filter(comp==F, condition=='set3_test' | condition=='set6_test') %>% 
...
```{r PART II: aggregate model fits}
...
```{r part A join model-behavioral data}
...
temp.count %>% 
  ...
  count() %>% 
   ggplot(aes(x=half1,y=n, group=half2_models,  fill=half2_models, label=n)) +
  # geom_tile()+
