...
plt.all.sims <- data.frame('accuracy'= c(RL.sim.dat[,1:12] %>% t() %>% c(), 
                                        LTM.sim.dat[,1:12] %>% t() %>% c(),
                                        RL_LTMpipe.sim.dat[,1:12] %>% t() %>% c(),
                                        RL_LTMstr.sim.dat[,1:12] %>% t() %>% c(),
                                        RL.sim.dat[,13:24] %>% t() %>% c(),
                                        LTM.sim.dat[,13:24] %>% t() %>% c(),
                                        RL_LTMpipe.sim.dat[,13:24] %>% t() %>% c(),
                                        RL_LTMstr.sim.dat[,13:24] %>% t() %>% c()
                           ...
                           'index'=rep(1:12,(nrow(RL.sim.dat) + nrow(LTM.sim.dat) + nrow(RL_LTMpipe.sim.dat) + nrow(RL_LTMstr.sim.dat)) * 2),
                           
                         'Models'=c(rep('RL set3', nrow(RL.sim.dat)*12 ),rep('LTM set3', nrow(LTM.sim.dat)*12), 
                                       rep('RL-LTM set3',nrow(RL_LTMpipe.sim.dat)*12), rep('RL-LTM biased set3', nrow(RL_LTMstr.sim.dat)*12),
                                       rep('RL set6', nrow(RL.sim.dat)*12),rep('LTM set6', nrow(LTM.sim.dat)*12), 
                                       rep('RL-LTM set6',nrow(RL_LTMpipe.sim.dat)*12), rep('RL-LTM biased set6', nrow(RL_LTMstr.sim.dat)*12)
                                       )
)


plt.all.sims %>% 
   summarySE( measurevar = "accuracy", groupvars = c("index", "Models")) %>% 
  ggplot(aes(factor(index), accuracy,color=Models, group=Models))+
   geom_point() +
   geom_line() +
   geom_errorbar(aes(ymin=accuracy-se, ymax=accuracy+se), width=.1) +
   #scale_color_manual(values = tmp.color)+ #
...
#              accuracy, 
#              color= Models %>% 
#                str_trunc(4,"right", ellipsis = "") %>% 
#                            as.factor(), 
#              shape = Models %>% 
#                str_trunc(4,"left", ellipsis = "") %>% 
#                            as.factor()), 
#          group = Models) +
#    geom_point() +
#    geom_line() +
