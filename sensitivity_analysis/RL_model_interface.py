...
#                   save averaged resluts from simulations along with parameters
    ...
    #changelog 01282023 saving all simulations before averaging for sensitivity test
        sim_data.append([temp3, temp6, temp_test3, temp_test6, bll, alpha, egs, imag, ans ])
...
def execute_sim(n,fromI,toI, frac):
    ...
    #sim_st.to_json('./expanded_sims/RL_model/RL_sim_std_date.JSON',orient='table') # this saves files to the new expanded sims folder

    sim.to_json('./RL_sims/RL_sim_data_' + 'frac_' +np.str(frac) +'_'+ np.str(fromI) + '_to_' + np.str(toI), orient='table')  # this saves files to the new expanded sims folder
